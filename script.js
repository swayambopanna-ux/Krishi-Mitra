...
async function fetchSensor() {
  try {
    ...
    setEl('sensor-timestamp', '⚠️ Cannot reach backend');
  }
}
...
async function loadPlantSelector() {
  try {
    ...
    const content = document.getElementById('irrigationContent');
...
async function loadIrrigationAdvice() {
  ...
  try {
    ...
    container.innerHTML = errorHTML('Cannot reach backend: ' + e.message);
  }
}
...
async function searchCrop(name) {
  ...
  try {
    const r = await fetch(`${CONFIG.BACKEND_URL}/api/crop-advisory/${encodeURIComponent(name)}`);
    const j = await r.json();
    const i = j.info;
    out.innerHTML = `
    ...
    out.innerHTML = errorHTML('Could not fetch: ' + e.message);
  }
}
...
async function runDetection() {
  ...
  out.innerHTML = loadingHTML('Analyzing image locally using TensorFlow.js (Zero API API Required)...');
  document.getElementById('detectBtn').disabled = true;
...
function setupFertilizer() {
  document.getElementById('fertBtn').addEventListener('click', async () => {
    ...
    try {
      const r = await fetch(`${CONFIG.BACKEND_URL}/api/fertilizer-advice?crop=${encodeURIComponent(crop)}&nitrogen=${n}&phosphorus=${p}&potassium=${k}`);
      const j = await r.json();
...
function setupSoilHealth() {
  document.getElementById('soilBtn').addEventListener('click', async () => {
    ...
    try {
      const r = await fetch(`${CONFIG.BACKEND_URL}/api/soil-health?ph=${ph}&organic_matter=${om}&moisture=${mo}`);
      const j = await r.json();
...
async function fetchMandi(commodity) {
  ...
  try {
    const r = await fetch(`${CONFIG.BACKEND_URL}/api/mandi?commodity=${encodeURIComponent(commodity)}`);
    ...
    out.innerHTML = `<div class="result-card">
      <h3>📊 Mandi API Setup</h3>
      <p style="line-height:1.8;color:var(--text-secondary)">
        ${e.message}<br><br>
        For reliable access: Register at <a href="https://data.gov.in" target="_blank"
        style="color:var(--primary-light)">data.gov.in</a> → 
        API key for "Current Daily Price of Various Commodities" → 
        replace <code style="background:rgba(255,255,255,0.05);padding:2px 6px;border-radius:4px">DATAGOV_KEY</code> in script.js
      </p>
    </div>`;
  }
}
